...
const CatererProfile = () => {
  ...
  const { addToCart, cartItem, setIsCartOpen } = useCart();
  ...
  return (
    <div className="min-h-screen pb-20" style={{ background: '#0a0a0f' }}>
      ...
      {/* ── HEADER HERO SECTION ── */}
      <div className="relative pt-32 pb-16 px-4 sm:px-6 lg:px-8 border-b border-white/5 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row gap-8 items-start justify-between">
          <div className="flex-1">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/5 backdrop-blur-md rounded-full border border-white/10 mb-6 shadow-xl">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-display font-medium text-white tracking-tight leading-none mb-4">
              {caterer.name}
            </h1>
            
            <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-gray-400 mb-8 font-medium">
          ...
          <div className="w-full md:w-auto flex flex-col gap-4 min-w-[280px]">
            <div className="bg-[#111116] border border-white/10 rounded-2xl p-6 shadow-2xl backdrop-blur-xl">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-16">
          ...
          {/* About Section */}
          <section>
            <h3 className="text-2xl font-display font-medium text-white mb-4">About {caterer.name}</h3>
          ...
          {caterer.images && caterer.images.length > 0 && (
            <section>
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-display font-medium text-white">Gallery Showcase</h3>
              ...
              <div className="flex overflow-x-auto gap-4 pb-6 snap-x snap-mandatory hide-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
                {caterer.images.map((img, i) => (
                  <div key={i} className="min-w-[85%] sm:min-w-[320px] max-w-[400px] h-[280px] snap-center rounded-2xl overflow-hidden shadow-xl border border-white/5 flex-shrink-0 group relative">
          ...
          {/* Menus Section */}
          <section>
            <h3 className="text-2xl font-display font-medium text-white mb-6">Available Menus</h3>
            {menus.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {menus.map((menu) => (
                  <div key={menu.id} className="bg-[#111116] border border-white/5 rounded-2xl p-6 group hover:border-amber-400/30 transition-colors flex flex-col h-full relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-start mb-4 relative z-10">
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                          <h4 className="text-xl font-bold text-white">{menu.menuName}</h4>
                    ...
                    <div className="mb-6 relative z-10">
                      <span className="text-2xl font-bold text-amber-400 drop-shadow-sm block mb-1">₹{menu.price}</span>
                      <span className="text-[10px] uppercase font-bold tracking-widest text-gray-500">Per Person</span>
                    </div>

                    <div className="mt-auto relative z-10">
                      <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-3 border-t border-white/5 pt-4">Included Dishes</p>
                      <ul className="space-y-2">
                        {menu.dishes.slice(0, 5).map((dish, i) => (
                          <li key={i} className="flex items-start text-sm text-gray-300">
                             <svg className="w-4 h-4 text-amber-400/70 mr-2 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                             <span className="leading-snug">{dish}</span>
                          </li>
                        ))}
                        {menu.dishes.length > 5 && (
                          <li className="text-xs font-medium text-amber-400 pl-6 mt-2">+ {menu.dishes.length - 5} more items</li>
                        )}
                      </ul>
                    </div>
                    
                    {cartItem?.menuId === menu.id ? (
                      <button
                        onClick={() => setIsCartOpen(true)}
                        className="mt-6 w-full py-2.5 bg-amber-400 hover:bg-amber-300 text-gray-950 text-sm font-bold tracking-wide rounded-xl transition-all relative z-10 flex items-center justify-center gap-2"
                      >
                      ...
                      <button
                        onClick={() => addToCart(menu, caterer)}
                        className="mt-6 w-full py-2.5 bg-white/5 hover:bg-amber-400/10 hover:border-amber-400/40 text-white hover:text-amber-400 text-sm font-semibold tracking-wide rounded-xl border border-white/10 transition-all duration-200 relative z-10 flex items-center justify-center gap-2"
                      >
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          <div className="bg-[#111116] border border-white/5 rounded-2xl p-6">
            <h4 className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-6 pb-4 border-b border-white/10">Specialized Cuisines</h4>
            <div className="flex flex-wrap gap-2">
          ...
          <div className="bg-[#111116] border border-white/5 rounded-2xl p-6">
            <h4 className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-6 pb-4 border-b border-white/10">Service Areas</h4>
          ...
          <div className="bg-[#111116] border border-white/5 rounded-2xl p-6 relative overflow-hidden">
             ...
             <h4 className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-4 relative z-10">Direct Contact</h4>
             <p className="text-white font-medium text-lg relative z-10 mb-6">{caterer.phone || 'Available upon request'}</p>
      ...
      {cartItem && cartItem.catererId === (caterer?.id || caterer?._id) && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-30 animate-fade-in">
          ...
          >
            ...
            <span>{cartItem.menuName} · View Cart</span>
            <span className="bg-gray-950/20 px-2 py-0.5 rounded-full text-xs font-bold">₹{cartItem.pricePerPerson}/pp</span>
          </button>
        </div>
      )}
