...
const CatererProfile = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-24 pb-12 flex items-center justify-center bg-gray-50 bg-gray-50">
  ...
  if (error || !caterer) {
    return (
      <div className="min-h-screen pt-24 pb-12 flex flex-col items-center justify-center text-center px-4 bg-gray-50 bg-gray-50">
        <svg className="w-16 h-16 text-gray-500 text-gray-500 mb-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        <h2 className="text-3xl font-display text-gray-900 text-gray-900 mb-2">{error || 'Caterer Not Found'}</h2>
        <p className="text-gray-600 text-gray-600 mb-8 max-w-md">We couldn't find the caterer you're looking for. The link might be broken or the caterer may have been removed.</p>
  ...
  return (
    <div className="min-h-screen pb-20 bg-gray-50 bg-gray-50 transition-colors">

      <div className="relative pt-24 pb-10 sm:pt-28 sm:pb-12 px-4 sm:px-6 lg:px-8 border-b border-gray-200 border-gray-200 overflow-hidden">
        <div className="absolute inset-0 bg-gray-50 bg-gray-50 z-0"></div>
        ...
        <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row gap-6 md:gap-8 items-start justify-between">
          <div className="flex-1">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-gray-100 bg-gray-100 backdrop-blur-md rounded-full border border-gray-200 border-gray-200 mb-4 shadow-xl">
              <span className="flex items-center text-amber-500 text-amber-500">
                {[...Array(5)].map((_, i) => (
              ...
              <span className="text-xs font-bold text-gray-900 text-gray-900 ml-1">{caterer.rating ? caterer.rating.toFixed(1) : '5.0'} / 5.0</span>
              <span className="text-gray-500 text-[10px] uppercase font-bold tracking-wider ml-1 pl-2 border-l border-gray-300 border-gray-300">EST. {caterer.establishedYear || '2010'}</span>
            </div>

            <h1 className="text-3xl sm:text-4xl md:text-5xl font-display font-medium text-gray-900 text-gray-900 tracking-tight leading-tight mb-3">
              {caterer.name}
            </h1>

            <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-gray-600 text-gray-600 mb-5 font-medium">
            ...
            <div className="flex flex-wrap gap-2">
              {caterer.services && caterer.services.map((svc, i) => (
                <span key={i} className="px-3 py-1 bg-gray-100 bg-gray-100 border border-gray-200 border-gray-200 text-gray-800 text-gray-900 rounded-lg text-xs font-medium tracking-wide">
                  {svc}
                </span>
              ))}
            </div>
          ...
          <div className="w-full md:w-[280px] md:shrink-0 flex flex-col">
            <div className="bg-white  border border-gray-200 rounded-2xl p-5 shadow-xl backdrop-blur-xl">
              <p className="text-gray-500 text-gray-600 text-sm mb-1 font-medium">Starting from</p>
              <div className="flex items-end gap-2 mb-5">
                <span className="text-4xl font-bold tracking-tight text-gray-900 text-gray-900 block">₹{caterer.pricePerPlate || '450'}</span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-10">
        ...
        <div className="lg:col-span-2 space-y-8 sm:space-y-10">
          ...
          {/* About Section */}
          <section>
            <h3 className="text-xl sm:text-2xl font-display font-medium text-gray-900 text-gray-900 mb-3">About {caterer.name}</h3>
            <p className="text-gray-600 text-gray-600 leading-relaxed text-[15px]">
          ...
          {caterer.images && caterer.images.length > 0 && (
            <section>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl sm:text-2xl font-display font-medium text-gray-900 text-gray-900">Gallery Showcase</h3>
                <div className="flex gap-2">
                  <button className="w-8 h-8 rounded-full border border-gray-300 border-gray-300 flex items-center justify-center text-gray-700 text-gray-900 hover:bg-gray-100 hover:bg-gray-100 transition-colors">
              ...
              <div className="flex overflow-x-auto gap-3 pb-4 snap-x snap-mandatory hide-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
                {caterer.images.map((img, i) => (
                  <div key={i} className="min-w-[80%] sm:min-w-[300px] max-w-[360px] h-[220px] sm:h-[260px] snap-center rounded-xl overflow-hidden shadow-xl border border-gray-200 border-gray-200 flex-shrink-0 group relative">
          ...
          {/* Menus Section */}
          <section>
            <h3 className="text-xl sm:text-2xl font-display font-medium text-gray-900 text-gray-900 mb-4">Available Menus</h3>
            {menus.length === 0 ? (
              <div className="p-6 bg-gray-50 bg-gray-50 border border-gray-200 border-gray-200 rounded-xl text-center">
                <p className="text-gray-600 text-gray-600">Menus are currently being updated by the caterer. Please inquire directly for options.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {menus.map((menu) => (
                  <div key={menu.id} className="bg-white  border border-gray-200 rounded-lg p-4 group hover:border-amber-400/30 transition-colors flex flex-col relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-start mb-2 relative z-10">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <h4 className="text-base font-bold text-gray-900 text-gray-900 truncate">{menu.menuName}</h4>
                          <span className={`w-2.5 h-2.5 rounded-full ${menu.type === 'Veg' ? 'bg-green-500' :
                    ...
                    <div className="mt-auto relative z-10">
                      <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-2 border-t border-gray-200 border-gray-200 pt-3">Dishes</p>
                      <ul className="space-y-1">
                        {menu.dishes.slice(0, 3).map((dish, i) => (
                          <li key={i} className="flex items-start text-xs text-gray-600 text-gray-600">
                    ...
                    {cartItems.some(item => item.menuId === menu.id) ? (
                      ...
                      <button
                        ...
                        className="mt-4 w-full py-2 bg-gray-100 bg-gray-100 hover:bg-amber-400/10 hover:border-amber-400/40 text-gray-900 text-gray-900 hover:text-amber-600 hover:text-amber-600 text-xs font-semibold rounded-lg border border-gray-200 border-gray-200 transition-all duration-200 relative z-10 flex items-center justify-center gap-1.5"
                      >
        ...
        <div className="lg:col-span-1 space-y-5 lg:space-y-6 flex flex-col lg:items-stretch">
          ...
          <div className="bg-white  border border-gray-200 rounded-xl p-5">
            <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4 pb-3 border-b border-gray-200 border-gray-200">Specialized Cuisines</h4>
            <div className="flex flex-wrap gap-2">
              {caterer.cuisines && caterer.cuisines.length > 0 ? caterer.cuisines.map((cuisine, i) => (
                ...
                <span className="text-gray-500 text-gray-600 text-sm">Multi-cuisine</span>
              )}
            </div>
          </div>

          <div className="bg-white  border border-gray-200 rounded-xl p-5">
            <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4 pb-3 border-b border-gray-200 border-gray-200">Service Areas</h4>
            <ul className="space-y-2">
              {caterer.areasServed && caterer.areasServed.length > 0 ? caterer.areasServed.map((area, i) => (
                <li key={i} className="flex items-center text-sm text-gray-600 text-gray-600">
                  <svg className="w-4 h-4 text-gray-500 text-gray-500 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                  {area}
                </li>
              )) : (
                <li className="text-gray-500 text-gray-600 text-sm">{caterer.city || caterer.location || 'Throughout the city'}</li>
              )}
            </ul>
          </div>

          <div className="bg-white  border border-gray-200 rounded-xl p-5 relative overflow-hidden">
            <div className="absolute -right-6 -bottom-6 opacity-20 text-gray-400 w-32 h-32">
            ...
            <p className="text-gray-900 font-medium text-base relative z-10 mb-4">{caterer.phone || 'Available upon request'}</p>
            <button className="w-full py-3 bg-gray-900 text-white font-bold rounded-xl relative z-10 flex justify-center items-center gap-2 hover:bg-gray-800 transition-colors shadow-xl">
